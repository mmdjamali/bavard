...
const PasswordInput : FC<props> = ({
    icon , placeholder , value , setValue , pass , sx
}) => {
        ...
        `}>
            ...
            onChange={(e : React.ChangeEvent<HTMLInputElement>) => {
                ...
                checkInput.current = setTimeout(() => {
                    if(pass) setValid(pass === targetValue)
                    else setValid(/.{8,}/.test(targetValue))
                    setFirstLoad(false)
                    setEmpty(false)
                },1000)
            }}
            ...
            onBlur={() => {
                ...
                if(pass) setValid(pass === value)
                else setValid(/.{8,}/.test(value))
            ...
            className='p-1.5 text-[1.5rem] cursor-pointer'>
                {visible ?
                    <MdVisibility/>
                    :
                    <MdVisibilityOff/>
